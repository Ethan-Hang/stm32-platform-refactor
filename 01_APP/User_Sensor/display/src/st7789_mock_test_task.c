...
//******************************* Functions *********************************//
static void mock_hex_dump(const char *label,
                          ...
                          uint32_t data_length)
{
    ...
    DEBUG_OUT(i, ST7789_MOCK_LOG_TAG, "%s len=%u : %s",
              label, (unsigned)data_length, buf);
}
...
static st7789_status_t mock_driver_bind(void)
{
    ...
    return bsp_st7789_driver_inst(&s_mock_driver,
                                  &s_mock_spi,
                                  &s_mock_timebase,
                                  &s_mock_os,
                                  &s_mock_panel);
...
static void case_report(const char *name, bool ok)
{
    ...
    else
    {
        ...
        DEBUG_OUT(e, ST7789_MOCK_LOG_TAG,
                  "[FAIL] %s  ret-path check failed (see stats above)",
                  name);
    }
    DEBUG_OUT(i, ST7789_MOCK_LOG_TAG,
              "       stats: tx=%u tx_dma=%u wait=%u last_dma_len=%u"
              " cs=%u dc=%u",
              (unsigned)s_stats.tx_count,
              (unsigned)s_stats.tx_dma_count,
              (unsigned)s_stats.wait_dma_count,
              (unsigned)s_stats.last_tx_dma_len,
              (unsigned)s_stats.last_cs,
...
static void test_case_addr_window_full(void)
{
    ...
    const uint32_t expected_tiles = (uint32_t)ST7789_MOCK_PANEL_WIDTH
                                  * (uint32_t)ST7789_MOCK_PANEL_HEIGHT
                                  * 2U
    ...
    st7789_status_t ret = s_mock_driver.pf_st7789_fill_region(
                                   &s_mock_driver, 0U, 0U,
                                   (uint16_t)(ST7789_MOCK_PANEL_WIDTH  - 1U),
                                   (uint16_t)(ST7789_MOCK_PANEL_HEIGHT - 1U),
                                   BLACK);
...
static void test_case_addr_window_sub(void)
{
    ...
    const uint32_t expected_tiles  = (total_bytes + ST7789_MOCK_TILE_BYTES - 1U)
                                   / ST7789_MOCK_TILE_BYTES;
...
static void test_case_addr_window_out_of_range(void)
{
    ...
    st7789_status_t ret = s_mock_driver.pf_st7789_fill_region(
                                   &s_mock_driver, 0U, 0U,
                                   ST7789_MOCK_PANEL_WIDTH,
                                   ST7789_MOCK_PANEL_HEIGHT,
                                   BLACK);
...
static void test_case_draw_pixel(void)
{
    ...
    stats_reset();
    st7789_status_t ret = s_mock_driver.pf_st7789_draw_pixel(
                                   &s_mock_driver, 50U, 50U, RED);
...
void st7789_mock_test_task(void *argument)
{
    ...
    if (ST7789_OK != bind_ret)
    {
        DEBUG_OUT(e, ST7789_ERR_LOG_TAG,
                  "bsp_st7789_driver_inst failed = %d", (int)bind_ret);
        for (;;)
        {
