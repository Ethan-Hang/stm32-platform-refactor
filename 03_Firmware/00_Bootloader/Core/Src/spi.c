...
// SPI1速度设置函数
// SPI速度=fAPB2/分频系数
// SPI_BaudRate_Prescaler: 范围
// SPI_BaudRatePrescaler_2~SPI_BaudRatePrescaler_256 fAPB2时钟一般为84MHz
...
void SPI1_SetSpeed(u8 SPI_BaudRate_Prescaler)
{
    ...
    assert_param(IS_SPI_BAUDRATE_PRESCALER(
        SPI_BaudRatePrescaler)); // 判断有效性
                                 // assert_param是STM32库函数判断有效性的函数，如果波特率预分频值不在上述的范围内，就会给程序员报错
    // 位带操作主要分两步，第一步将所要操作的寄存器的某一位清零
    // 第二步将寄存器清零的相关位写入所需的值
    SPI1->CR1 &= 0xFFC7; // 0xFFC7对应于 1111 1111 1100
                         // 0111，相与也就表示将CR1寄存器的3-5位清零
    SPI1->CR1 |=
